...
Collagen is a data augmentation program that can generate and
visualize images representing sequences of numbers.  The shapes of the
digits are either taken directly from a dataset like
[MNIST](http://yann.lecun.com/exdb/mnist/), or generated by augmenting
the shapes found in some dataset. The generated images then can be
used to train classifiers and generative deep learning models.

Beside a python package Collagen contains a command line tool for
generating, saving and inspecting examples of digit sequences.  It is
helpful for experimenting with image augmentation pipelines and for
inspecting the characteristics of the generated images.  Finally it
can be used for evaluating the the models trained with datasets
generated by Collagen or some other tool.
...
homepage of [Collagen](http://newskylabs/collagen):
[http://newskylabs/collagen](http://newskylabs/collagen).
